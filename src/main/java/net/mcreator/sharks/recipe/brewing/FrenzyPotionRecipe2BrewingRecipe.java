...
public class FrenzyPotionRecipe2BrewingRecipe implements IBrewingRecipe {
   ...
   public static void init(RegisterBrewingRecipesEvent event) {
      event.getBuilder().addRecipe(new FrenzyPotionRecipe2BrewingRecipe());
   }
   ...
   public boolean isIngredient(ItemStack ingredient) {
      return Ingredient.of(new ItemStack[]{new ItemStack((ItemLike)DwurdySharksModItems.RAW_BARRACUDA.get())}).test(ingredient);
   }

   public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
      return this.isInput(input) && this.isIngredient(ingredient)
         ? PotionContents.createItemStack(input.getItem(), DwurdySharksModPotions.FRENZY_POTION)
         : ItemStack.EMPTY;
   }
}
